%% ========================================================================
% STEP A: SOLVE THE FIVE-YEAR GLOBAL DICE-2016R BASELINE
%
% Optimises global emissions abatement and savings, generates the global
% climate and economic pathway, and saves the results subsequently used as
% the fixed global pathway in the annual regional model.
% ========================================================================
...
% Preserve the original app behaviour while avoiding an unconditional
% clear that deletes running_app before it can be checked.
...
%% =====================================================================
%% MODEL CONFIGURATION, PARAMETERS AND OUTPUT DIRECTORIES

% User options
customdir = '2026-07_DICE_BASELINE';
Params.timestep = 5;   % years per model period

model_start_year = 2015;
model_end_year   = 2300;  

assert(mod(model_end_year-model_start_year, Params.timestep) == 0, ...
    'The requested end year must lie on the selected model timestep.');

% Lemoine defines horizon = numyears/timestep and constructs periods from
% 2015 to 2015 + numyears - timestep. This expression includes end_year.
Params.numyears = model_end_year - model_start_year + Params.timestep;
...
Params.transitionsasconstraints = 0; % =0: solve by guessing policy and simulating trajectories; =1 (default): solve by also guessing states and treating transition equations as constraints
...
% Parameterization
assert(mod(Params.numyears, Params.timestep) == 0, ...
    'Params.numyears must be divisible by Params.timestep.');

% Run Lemoine parameterization script
run sub_parameters

% Small positive floor to avoid zero or negative consumption in utility.
...
% optimize
dosolve = 2; % initial solve plus one SQP fallback
elapsedtime = 0;
tic;
while dosolve>0
    if Params.useknitro == 1
        ...
        if exitflag<=0
            ...
            if dosolve > 0
                guess = out_controls;
                % Try SQP without changing the derivative contract used
                % by the initial trajectory-based solve.
                Params.optimize_options_list = optimoptions(@fmincon, ...
                    'Algorithm','sqp', ...
                    'StepTolerance',1e-10, ...
...
if exitflag <= 0
    diary off;
    error('DICE optimisation failed after both attempts. Exit flag: %d', exitflag);
end

% Store constraints at the accepted solution
...
%
%  This section:
%     - Extracts the optimised abatement and savings paths.
%     - Simulates the deterministic DICE-2016R baseline trajectory.
%     - Calculates key outputs: output, consumption, capital, emissions,
%       carbon stocks, temperature, abatement costs, welfare, and SCC.
...
% Welfare-equivalent consumption loss from a one-GtC pulse in each period.
...
for tstep = 1:Params.horizon
    ...
    % Fun.otherems is an annual flow and trajectory multiplies it by
    % Params.timestep. Divide by timestep to create a one-GtC total pulse.
    Fun.otherems = @(t) ...
        Params.otherems0 .* ...
        (1-Params.gotherems).^((t-1).*Params.timestep/5) + ...
        (pulse_GtC/Params.timestep) .* (t == tstep);

    [C_margems, ~, ~, ~, ~, ~, ~, ~, ~] = ...
        trajectory(out_policy, 1, Params.horizon, Fun, Params);

    W1e = sum(Params.discfactor .* Params.pop .* ...
        Fun.utility(C_margems, Params.pop));

    dWc = Params.discfactor(tstep) .* Params.pop(tstep) .* ...
        Fun.dutility_dC(C(tstep), Params.pop(tstep));

    % Trillion USD/GtCO2 -> USD/tCO2: multiply by 1000.
    SCC_pertCO2(tstep,1) = ...
        -((W1e-W0)/dWc) .* 1000 ./ ...
        (Params.co2_per_c .* pulse_GtC);
end

% Restore the baseline non-industrial emissions function.
Fun.otherems = @(t) Params.otherems0 .* ...
    (1-Params.gotherems).^((t-1).*Params.timestep/5);

clear W1e W0 dWc C_margems pulse_GtC;
...
%% ========================================================================
%  CLEAN DICE-2016R BASELINE OUTPUTS
%  ========================================================================
%
%  This section:
%     - Aligns deterministic DICE-2016R outputs to the model horizon.
%     - Creates clean baseline variables for temperature, output,
%       emissions, carbon, abatement, savings, consumption, and capital.
%     - Calculates economic loss and abatement cost as % of gross output.
%  ========================================================================

H = Params.horizon;

% Time axis
year = model_start_year + (0:H-1)' .* Params.timestep;

% Force all main deterministic vectors to match the model horizon.
T_base = T(1:H);
Tocean_base = Tocean(1:H);
Ygross_base = Ygross(1:H);
Ynet_base = Ynet(1:H);
abatecost_base = abatecost(1:H);
emsind_base = emsind(1:H);
C_base = C(1:H);
K_base = K(1:H);
abaterate_base = abaterate(1:H);
savingsrate_base = savingsrate(1:H);

if exist('Carbon_ppm','var')
    Carbon_ppm_base = Carbon_ppm(1:H);
else
    Carbon_ppm_base = NaN(H,1);
end

% Economic output loss in model output units.
EconomicLoss_base = Ygross_base - Ynet_base;

% Economic loss as percentage of gross output/GDP.
EconomicLossPct_base = 100 .* EconomicLoss_base ./ Ygross_base;

% Abatement cost as percentage of gross output.
AbateCostPct_base = 100 .* abatecost_base ./ Ygross_base;

% Control variables as percentages.
AbateRatePct_base = 100 .* abaterate_base;
SavingsRatePct_base = 100 .* savingsrate_base;
...
% Figure 1: Global DICE optimal pathways
Ygross_growth_pct = NaN(H,1);
Ygross_growth_pct(2:end) = 100 .* ...
    ((Ygross_base(2:end) ./ Ygross_base(1:end-1)) ...
    .^(1/Params.timestep) - 1);

   
figure_A1 = figure('Name','DICE Optimal Pathways', ...
...
titles = { ...
    ...
    'Atmospheric CO2', ...
    'Economic Output', ...
    'Annualised Economic Growth'};

y_labels = { ...
    '2010 USD/tCO2', ...
    'Gt C per model period', ...
    'Degrees C', ...
    'ppm', ...
    'Trillion 2010 USD', ...
    '% per year'};
...
for ii = 1:6
    ...
    if ii == 5
        legend('Gross', 'Net of climate damages', ...
            'Location', 'best');
    end
end

sgtitle('Global DICE-2016R Optimal Pathways');
exportgraphics(figure_A1,fullfile(plots_dir, ...
    'Figure_A1_DICE_optimal_pathways.png'),'Resolution',300);
savefig(figure_A1,fullfile(plots_dir, ...
    'Figure_A1_DICE_optimal_pathways.fig'));

dice_results_file = fullfile(maindir, ...
    'DICE2016R_baseline_results.mat');

save(dice_results_file, ...
    'year', ...
    'T_base', ...
    'abaterate_base', ...
    'savingsrate_base', ...
